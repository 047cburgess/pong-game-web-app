...
import { APP } from "../App";
import { AElement, Div, Paragraph, Button, Header, Image, Textbox } from "./elements/Elements";
import { AVATAR_DIV, DEFAULT_BUTTON, HOW_TO_CENTER_A_DIV, INPUT_BOX_OUTLINE, INPUT_BOX_RED_OUTLINE } from "./elements/CssUtils";
...
export class WaitingMenu extends Div {
	...
	private header: Header = new Header(2, "Custom Game").class("text-3xl font-bold m-0 mb-6 text-center") as Header;
	private playerContainer: Div = new Div().withStyle("width:45%; height:45%; ").class(this.PLAYERCONTAINER_CLASS).withId("players-container") as Div;
	private inplayerContainer: AElement[] = [];

	constructor() {
		super();
		this.addContent(new Div(
			this.header,
			this.playerContainer).class(`
...
export class InviteMenu extends Div {
	...
	private mockFriends: UserInfo[] = Array.from({ length: 20 }, (_, i) => ({
		id: i + 1,
		username: `Friend${i + 1}`,
		lastSeen: new Date(Date.now() - Math.random() * 4 * 60 * 1000).toISOString()
	}));

	private friendElements: AElement[] = [];
	private header: Header = new Header(2, "Invite Players")
		.class("text-3xl font-bold m-0 mb-6 text-center") as Header;
	...
	async onEnterSearch(value: string) {
		...
		if(!resp.ok){
			this.textbar.remove_class(INPUT_BOX_OUTLINE);
	...
	updatefriendlist() {
		...
		const onlineFriends = friends.filter(friend =>
			!friend.lastSeen || (Date.now() - new Date(friend.lastSeen).getTime()) <= 5 * 60 * 1000
		);
		...
		const newfriendElements: AElement[] = onlineFriends.map(friend =>
			new Div(
				...
				new Paragraph(friend.username)
					...
					.withStyle("flex-grow: 1; margin-left: 10px; overflow-hidden; text-ellipsis;")
			).class("flex items-center justify-start p-2 hover:bg-gray-700 rounded-lg mx-auto")
...
export class CustomGamePage extends Page {
	...
	private mainContents: Div = new Div().withId("custompage-main-container") as Div;
	...
	constructor(router: Router, isHost = false) {
		...
		this.isHost = isHost;
		if (isHost) this.playerSlots[0] = { id: this.currentUserId, name: APP.userInfo?.username, avatar: "placeholder_avatar.png", isHost: true };
		this.PlayerCards.set("player-0", new PlayerCard(APP.userInfo?.username, this.onClick.bind(this)).withId("player-0").withStyle("top: 10px; left: 10px; width:45%; height:45%;").class("absolute") as PlayerCard)
		this.PlayerCards.set("player-1", new PlayerCard(undefined, this.onClick.bind(this)).withId("player-1").withStyle("top: 10px; right: 10px; width:45%; height:45%;").class("absolute") as PlayerCard);
		this.PlayerCards.set("player-2", new PlayerCard(undefined, this.onClick.bind(this)).withId("player-2").withStyle("bottom: 10px; left: 10px; width:45%; height:45%;").class("absolute") as PlayerCard);
		this.PlayerCards.set("player-3", new PlayerCard(undefined, this.onClick.bind(this)).withId("player-3").withStyle("bottom: 10px; right: 10px; width:45%; height:45%;").class("absolute") as PlayerCard);
	...
	render() {
		...
		if (!el) { console.log("should not happen"); return; }
...
export class PlayerCard extends Div {
	...
	private waitingParagraph: Paragraph = new Paragraph("Waiting...").class("text-white font-bold text-xs break-words").withStyle("max-width: 80%;") as Paragraph;
	private overlayPlus: Button = new Button(
		new Div(
			new Paragraph("+").class(`text-white text-4xl font-bold absolute inset-0 flex items-center justify-center z-10`
	...
	private mainContents: Div = new Div().withId("mainContents" + this._id).class(this.BACKGROUND_STYLE).class(this.CONTENT_STYLE) as Div;
	...
	constructor(playerName: string = "Waiting...", private onClick?: () => void) {
		super();
	...
	setPlayer(name: string) {
		...
		this.mainContents.addContent([new Div(
			new Image("/api/v1/user/avatars/" + name + ".webp").class("absolute z-0")
		).class(AVATAR_DIV),
		new Paragraph(name)
	...
	forceMouseLeave() {
		this.mainContents.remove_class("blur-xs");
