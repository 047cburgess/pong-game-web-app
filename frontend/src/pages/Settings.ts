...
import { AVATAR_DIV, EVIL_RED_BUTTON, HOW_TO_CENTER_A_DIV, INPUT_BOX_OUTLINE } from "./elements/CssUtils";
import { AElement, Button, Div, Paragraph, Textbox, Image, Inline } from "./elements/Elements";
...
export default class SettingsPage extends Page {
  ...
  readonly fileInput: AElement =
    new Inline(`<input id="${this.fileInputId}" type="file" name="name" style="display:none;"/>`)
    ...
    return [
      new Div(
        new Div(
          this.avatarDiv,
          new Button(
            new Paragraph("Change avatar").class("text-xl p-2 select-none self-center text-center")
        ...
        new Div(
          new Div(
            ...
            new Paragraph("(NOT USED FOR SIGN IN)").class("text-xl text-red-900"),
            this.usernameText
    ...
    if (fileInput) {
      fileInput.onchange = async (e) => {
        ...
        if (!resp.ok) {
          ...
          alert("Failed to upload new avatar: " + resp.status + (text && "\n" + text));
        } else {
          this.avatarDiv.contents[0] = new Image("/api/v1/user/avatars/" + APP.userInfo?.username + ".webp");
  ...
  async updateUsername() {
    const resp = await fetch("/api/v1/user/username", {
      method: "PUT",
      body: JSON.stringify({ username: (this.usernameText.byId() as HTMLInputElement).value }),
      headers: {
    ...
    const info: SelfInfo | null = (respInfo.ok || respInfo.status === 304)
      ? await respInfo.json().catch(console.error) : null;
    if (info) {
      APP.onLogin(info);
    } else {
      APP.onLogout();
  ...
  async deleteAccount() {
    const conf = confirm("You are about to delete your account.\nThis action is final\n(and will have dire consequences in the future)");
    if (!conf) return;
